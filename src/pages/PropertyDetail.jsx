      ...
      <div className="bg-white dark:bg-secondary-900 shadow border-b border-light">
        <div className="container py-4">
          <div className="flex items-center space-x-2 text-sm">
            ...
            <Link to="/properties" className="text-link">Properties</Link>
            <span className="text-muted">/</span>
            <span className="icon-primary">{property.title}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div
              ...
              className="card"
            >
              <h2 className="text-2xl font-bold mb-4 text-heading">Property Details</h2>
              <p className="text-body mb-6">{property.description}</p>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="flex items-center space-x-2 text-body">
                  {/* <FiBed className="icon-primary" /> */}
                  <span>{property.parkingSpaces} Parking</span>
                </div>
                <div className="flex items-center space-x-2 text-body">
                  <FiMaximize2 className="icon-primary" />
                  <span>{property.lotSize}</span>
                </div>
                <div className="flex items-center space-x-2 text-body">
                  <FiCalendar className="icon-primary" />
              ...
              <h3 className="text-xl font-semibold mb-4 text-heading">Features</h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                {property.features.map((feature, index) => (
                  <div key={index} className="flex items-center space-x-2 text-body">
              ...
              {/* Token Details */}
              <h3 className="text-xl font-semibold mb-4 text-heading">Token Information</h3>
              <div className="bg-secondary-50 dark:bg-secondary-700 rounded-lg p-6 mb-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-semibold text-heading">{property.tokenDetails.tokenSymbol}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted">Token Price</p>
                    <p className="font-semibold text-heading">{property.tokenDetails.tokenPrice}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted">Available Tokens</p>
                    <p className="font-semibold text-heading">{property.tokenDetails.availableTokens.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted">Total Supply</p>
                    <p className="font-semibold text-heading">{property.tokenDetails.totalTokens.toLocaleString()}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-sm text-muted">Smart Contract</p>
                    <p className="font-mono text-sm text-heading">{property.tokenDetails.contractAddress}</p>
                  </div>
                </div>
              </div>

              {/* Financial Details */}
              <h3 className="text-xl font-semibold mb-4 text-heading">Financial Overview</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-secondary-50 dark:bg-secondary-700 rounded-lg p-6">
                  <h4 className="font-semibold mb-4 text-heading">Rental Income</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-body">Gross Rent</span>
                      <span className="font-medium text-heading">{property.financials.grossRent}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-body">Net Rent</span>
                      <span className="font-medium text-heading">{property.financials.netRent}</span>
                    </div>
                  </div>
                </div>
                <div className="bg-secondary-50 dark:bg-secondary-700 rounded-lg p-6">
                  <h4 className="font-semibold mb-4 text-heading">Expenses</h4>
                  <div className="space-y-2">
                    {Object.entries(property.financials.expenses).map(([key, value]) => (
                      <div key={key} className="flex justify-between">
                        <span className="text-body">{key.replace('_', ' ').charAt(0).toUpperCase() + key.slice(1)}</span>
          ...
          >
            ...
            <div className="card">
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-2xl font-bold text-heading">${property.price.usd.toLocaleString()}</span>
                  </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between">
                  <span className="text-body">Rental Yield</span>
                  <span className="font-medium text-heading">{property.metrics.rentalYield}</span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="card">
              <div className="flex items-center space-x-4 mb-4">
                ...
                />
                <div>
                  <h3 className="font-semibold text-heading">{property.agent.name}</h3>
                  <p className="text-sm text-body">Investment Advisor</p>
                </div>
              </div>
              <div className="space-y-2">
